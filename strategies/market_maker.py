...
from api.apex_exchange_client import ApexExchangeClient
...
from strategies.components.stats import (
    calculate_session_profit as _cmp_calculate_session_profit,
    calculate_average_buy_cost as _cmp_calculate_average_buy_cost,
)
from strategies.components.order_ops import (
    cancel_orders_diff as _cmp_cancel_orders_diff,
    build_limit_order as _cmp_build_limit_order,
)
...
class MarketMaker:
    def __init__(
        ...
        if exchange == 'backpack':
            ...
            self.client = ApexExchangeClient(
                account_label=self.exchange_config.get('account_label'),
                account_index=self.exchange_config.get('account_index')
            )
        else:
        ...
        if exchange == 'backpack':
            ...
            # Apex 使用轮询方式获取订单更新
            self.ws = None
            logger.info("Apex交易所使用轮询方式进行订单状态更新")
        else:
    ...
    def _init_components(self):
        ...
        self.mm_context = MMContext(
            ...
            base_spread_percentage=self.base_spread_percentage,
            ...
            # 运行期引用（指向 MarketMaker 的实例属性）
            active_buy_orders=self.active_buy_orders,
            active_sell_orders=self.active_sell_orders,
            session_buy_trades=self.session_buy_trades,
            session_sell_trades=self.session_sell_trades,
            ws=self.ws,
            ws_proxy=self.ws_proxy,
        )

        # 初始化各组件
        self.price_engine = PriceEngine(self.mm_context)
        self.order_manager = OrderManager(self.mm_context)
        self.inventory_balancer = InventoryBalancer(self.mm_context)
        self.stats_recorder = StatsRecorder(self.mm_context)
        self.ws_manager = WSManager(self.mm_context)
        self.order_dispatcher = OrderUpdateDispatcher(self.mm_context)
        self.risk_manager = RiskManager(self.mm_context)
        self.strategy_runner = StrategyRunner(self.mm_context)

        logger.debug("组件骨架初始化完成（PR-1）")
    ...
    def get_total_balance(self):
        """获取总余额，包含普通余额和抵押品余额（委托给 InventoryBalancer）"""
        return self.inventory_balancer.get_balances()

    def get_asset_balance(self, asset):
        """获取指定资产的总可用余额（委托给 InventoryBalancer）"""
        return self.inventory_balancer.get_asset_balance(asset)
    ...
    def _load_trading_stats(self):
        """从数据库加载交易统计数据（委托给 StatsRecorder）"""
        self.stats_recorder.load_today_stats()
    
    def _load_recent_trades(self):
        """从数据库加载历史成交记录（委托给 StatsRecorder）"""
        self.stats_recorder.load_recent_trades()
    ...
    def _calculate_db_profit(self):
        """基于数据库记录计算已实现利润（委托给 StatsRecorder）"""
        return self.stats_recorder.calculate_db_profit()
    
    def _update_trading_stats(self):
        """更新每日交易统计数据（委托给 StatsRecorder）"""
        self.stats_recorder.flush_daily_stats()
    
    def _calculate_average_buy_cost(self):
        """计算平均买入成本"""
        fallback = None
        if self.ws and self.ws.connected and getattr(self.ws, 'bid_price', None):
            fallback = self.ws.bid_price
        return _cmp_calculate_average_buy_cost(self.buy_trades, self.sell_trades, fallback)
    
    def _calculate_session_profit(self):
        """计算本次执行的已实现利润（委托给 StatsRecorder）"""
        return self.stats_recorder.get_session_profit()
    ...
    def get_current_price(self):
        """获取当前价格（委托给 PriceEngine）"""
        self.check_ws_connection()
        return self.price_engine.get_current_price()

    def get_market_depth(self):
        """获取市场深度（委托给 PriceEngine）"""
        self.check_ws_connection()
        return self.price_engine.get_market_depth()
    ...
    def calculate_prices(self):
        """计算买卖订单价格（委托给 PriceEngine）"""
        return self.price_engine.calculate_prices()
    
    def need_rebalance(self):
        """判断是否需要重平衡仓位（委托给 InventoryBalancer）"""
        return self.inventory_balancer.need_rebalance()
    
    def rebalance_position(self):
        """重平衡仓位（委托给 InventoryBalancer）"""
        self.check_ws_connection()
        return self.inventory_balancer.rebalance()
    ...
    def place_limit_orders(self):
        """下限价单（使用总余额包含抵押品）（委托给 OrderManager）"""
        self.check_ws_connection()
        ...
        # 委托 OrderManager 下单
        result = self.order_manager.place_grid(buy_prices, sell_prices, buy_quantity, sell_quantity)
        # 更新统计
        self.orders_placed += result.get("buy_count", 0) + result.get("sell_count", 0)
    
    def cancel_existing_orders(self):
        """取消所有现有订单（委托给 OrderManager）"""
        self.order_manager.cancel_all()

    def cancel_orders_diff(self, keep_ids=None):
        """差分撤单（委托给 OrderManager）"""
        success = self.order_manager.cancel_diff(keep_ids)
        self.orders_cancelled += int(success or 0)
    
    def check_order_fills(self):
        """检查订单成交情况（委托给 OrderManager）"""
